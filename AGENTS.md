...
2. **Contract drift (frontend ↔ server) — resolved by A3 (2026-09-23).** History: The frontends call endpoints `server/app.py` does **not** implement at HEAD: the session-based Incoming Inventory wizard (`/api/incoming/scan_page|confirm_job|pallet_photo|finalize`, `/api/inventory/pms`, `/api/schedule/warehouse/ready_to_pack`, `/send_to_pm`, `file/<ticket_filename>`) vs. server's single-shot `/api/incoming/scan|confirm|flag` + `/send_copy_to_pm` + `file/<int>`. Mapping table in `specs/001-server/contracts/api.md`. Do not "fix" one side without the other.
